...
export function testIfBothAreEquivalent(): void {
    fs.readFile('./data/perCounty.json', (e1, d1) => {
        fs.readFile('./data_old/perCounty.json', (e2, d2) => {
            ...
            const dataCurrent = parse(d1.toString()) as Map<number, Map<number, RKIData>>;
            ...
            if (keysCurrent.length != keysOld.length) console.log('different key length');
            ...
            keysCurrent.forEach(k => {
                ...
                if (historyDataCurrent.size < historyDataOld.size) console.log('less data now', k, historyDataCurrent.size, historyDataOld.size);

                historyDataOld.forEach((rkiData, date) => {
                    const MS_PER_DAY = 1000 * 60 * 60 * 24;
                    // we shifted one full day back
                    if (!historyDataCurrent.has(date - MS_PER_DAY)) console.log('date-key not found', date);
                    else {
                        ...
                        if (!identical) {
                            if (rkiData.Incidence7 === -1 && new Date(Date.parse(rkiData.Date as any as string)) < new Date(Date.parse('2020-04-01T00:00:00Z'))) {
                                // Ignore this false positive here, because previously the 7-days-incidence could not be calculated for the very first days
                            }
                            else if(rkiData.StateId === 0 && rkiData.CountyId === 0 && rkiData.Population < 83_100_000) {
                                // Data for whole Germany; Ignore if population is not the full population of Germany
                            }
                            else {
                                console.log('different RKIData entries', date, currentRKIData, rkiData);
                                differingCount++;
                            }
                        }
                    }
